...
import { YearMovieItem, getYearMoviesByPopularity } from '@/app/services/years';
...
export default function YearFilmsScreen() {
  ...
  const parsedYear = Number(params.year ?? '0');
  ...
  const [movies, setMovies] = useState<YearMovieItem[]>([]);
  ...
  const loadMovies = useCallback(
    async (offset: number, isLoadMore: boolean) => {
      ...
      try {
        ...
        const nextBatch = await getYearMoviesByPopularity(
          db,
          parsedYear,
          PAGE_SIZE,
          offset,
        ...
        ListEmptyComponent={
          <View style={styles.centered}>
            {loading ? (
              ...
              <Text style={styles.emptyText}>No movies for this year.</Text>
            )}
          </View>
        }
